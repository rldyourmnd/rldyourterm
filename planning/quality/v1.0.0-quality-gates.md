...
- Monitor-driven frame pacing must follow current display refresh and screen moves (60/144 Hz) must not cause render corruption.
...
- Перенос окна между 60/144Hz мониторами не вызывает падения, и каденс обновляется под текущий монитор.
