...
- Impact: High
- Likelihood: Medium
- Owner: foundation/api::pty + services/session
- Status: Mitigated
- Mitigation:
...
- Impact: Medium
- Likelihood: Medium
- Owner: foundation/api::window + services/render
- Status: Mitigated
- Mitigation:
  - resize coalescing and debounce
  - adaptive redraw throttling
  - font/cell metrics recompute only on settled resize
  - monitor-driven cadence re-sync on monitor transfer without restart
...
- Impact: Medium
- Likelihood: Low
- Owner: features/shell_integration
- Status: Mitigated
- Mitigation:
...
## Risk 9: Monitor refresh metadata unavailable or unstable
- Impact: Medium
- Likelihood: Medium
- Owner: foundation/api::window + services/render_pacing
- Status: Mitigated
- Mitigation:
  - refresh-rate query through window adapter on every monitor transition event
  - safe fallback path if refresh-rate is unknown (no crash, no forced hardcoded fps)
  - diagnostics events for refresh changes and cadence updates
